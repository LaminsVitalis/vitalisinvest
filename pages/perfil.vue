<template>
    <div class="min-h-screen bg-gradient-to-b from-blue-600 to-blue-800 p-4 pb-20">
      ...
      <div class="bg-white/10 backdrop-blur-md rounded-lg p-6 mb-6">
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p class="text-white/80">{{ usuario.nivel }}</p>
          </div>
        </div>
        <div class="space-y-2">
      ...
      <div v-if="mostrarModalEdicao" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div class="bg-gradient-to-b from-blue-700 to-blue-900 rounded-lg p-6 w-full max-w-md shadow-xl">
  ...
  const usuarioEditado = ref({ ...usuario, senha: '', confirmarSenha: '' })
  ...
  const editarPerfil = () => {
    usuarioEditado.value = { ...usuario, senha: '', confirmarSenha: '' }
  ...
  const salvarPerfil = () => {
    ...
    // Aqui você pode adicionar validação adicional e lógica para salvar as alterações no backend
